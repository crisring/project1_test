<%@page import="manage.inquiry.InquiryUtil"%>
...
<jsp:useBean id="sVO" class="user.vo.InquirySearchVO" scope="page" />
...
<%}%>
	...
	function openPopup(inquiryId) {
		...
		var width = 700;
		var height = 700;
		window.open("admin_inquiry_popup.jsp?inquiryId=" + inquiryId,
				"admin_inquiry_popup", "width=" + width + ",height=" + height
						+ ",left=" + left + ",top=" + top);
	}
</script>
</head>
...
<body>
	...
	<div id="main-content">
		...
		<!-- 검색 필터 폼 -->
		<div class="form">
			<form method="GET" action="admin_inquiry.jsp" name="searchFrm"
				id="searchFrm">
				<!-- 현재 페이지를 관리하기 위한 숨겨진 필드 -->
				<input type="hidden" id="currentPage" name="currentPage" value="1">

				<div class="form-group">
					<label for="filter">유형</label> <select id="filter" name="filter"
						class="form-select">
						<option value="전체"
							<c:if test="${sVO.filter eq '전체'}">selected</c:if>>전체</option>
						<option value="상품"
							<c:if test="${sVO.filter eq '상품'}">selected</c:if>>상품문의</option>
						<option value="취소"
							<c:if test="${sVO.filter eq '취소'}">selected</c:if>>구매취소</option>
						<option value="배송"
							<c:if test="${sVO.filter eq '배송'}">selected</c:if>>배송</option>
					</select>
				</div>
				...
				<div class="form-group">
					...
					<div class="btn-group" role="group">
						<!-- 오늘 -->
						<!-- 전체 -->
						<input type="radio" class="btn-check" name="date" id="btnradio0"
							value="all" autocomplete="off"
							<c:if test="${param.date eq 'all' || empty param.date}">checked</c:if>>
						<label class="btn btn-outline-secondary btn-sm" for="btnradio0"
							style="margin-right: 0px; width: 70px">전체</label> <input
							type="radio" class="btn-check" name="date" id="btnradio1"
							value="today" autocomplete="off"
							<c:if test="${param.date eq 'today'}">checked</c:if>> <label
							class="btn btn-outline-secondary btn-sm" for="btnradio1"
							style="margin-right: 0px; width: 70px">1일</label>
						<!-- 지난 3일 -->
						<input type="radio" class="btn-check" name="date" id="btnradio2"
							value="last3days" autocomplete="off"
							<c:if test="${param.date eq 'last3days'}">checked</c:if>>
						<label class="btn btn-outline-secondary btn-sm" for="btnradio2"
							style="margin-right: 0px; width: 70px">3일</label>

						<!-- 지난 1주일 -->
						<input type="radio" class="btn-check" name="date" id="btnradio3"
							value="lastweek" autocomplete="off"
							<c:if test="${param.date eq 'lastweek'}">checked</c:if>>
						<label class="btn btn-outline-secondary btn-sm" for="btnradio3"
							style="margin-right: 0px; width: 70px">1주일</label>

						<!-- 지난 1달 -->
						<input type="radio" class="btn-check" name="date" id="btnradio4"
							value="lastmonth" autocomplete="off"
							<c:if test="${param.date eq 'lastmonth'}">checked</c:if>>
						<label class="btn btn-outline-secondary btn-sm" for="btnradio4"
							style="margin-right: 0px; width: 70px">1달</label>
					</div>
					<input type="date" id="startDate" name="startDate"
						class="form-control date" value="${sVO.startDate}"
						style="margin-left: 10px"> ~ <input type="date"
						id="endDate" name="endDate" class="form-control date"
						value="${sVO.endDate}" style="margin-left: 10px">
				</div>

				<div class="form-group">
					<label for="search">검색</label> <select name="field" id="field"
						class="form-select">
						...
						style="width: 250px" value="${sVO.keyword}" class="form-control"
						placeholder="검색할 키워드를 입력하세요" />
				</div>

				<div class="form-group"
					style="justify-content: center; margin-top: 50px;">
					<input type="button" id="searchBtn" class="btn btn-success"
						value="검색"> <input type="reset" class="btn btn-light"
						value="옵션 초기화">
				</div>
			</form>
		</div>
		...
		<div id="searchResults">
			...
			<div class="form">
				<h5 class="form-group" style="border-bottom: 1px solid #EEF0F4">
					검색결과 :
					<c:out value="${sVO.totalCount}" />
					건
				</h5>
				<table class="table table-striped table-hover">
					<c:if test="${empty listBoard }">
						<tr>
							<td colspan="6" style="text-align: center;"><strong>해당
									조건에 맞는 검색 결과가 없습니다 </strong></td>
						</tr>
					</c:if>
						...
						<c:forEach var="iVO" items="${listBoard}" varStatus="i">
							<tr>
								...
								<td><a href="#" onclick="openPopup('${iVO.inquiryId}')">
										<c:out value="${iVO.title}" />
								</a></td>
								<td><c:out value="${iVO.status}" /></td>
								<td><fmt:formatDate value="${iVO.createAt}"
				...
				<p style="color: E9ECEF; font-size: 12px">제목 클릭시 문의 상세보기 페이지가
					나타납니다.</p>
